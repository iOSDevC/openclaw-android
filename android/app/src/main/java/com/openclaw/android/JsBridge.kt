...
class JsBridge(
    ...
    private fun requireCapability(
        ...
        privilegeRequired: Boolean = false,
    ) {
        if (!activity.isTrustedWebContentActive()) {
            throw SecurityException("Bridge access is limited to trusted app content.")
        }
        ...
        if (privilegeManager.isPrivileged()) {
            allowedCapabilities += Capability.RUN_ARBITRARY_COMMAND
        }
        if (capability !in allowedCapabilities) {
            throw SecurityException("Capability $capability is not available right now.")
        }
        if (privilegeRequired) {
            privilegeManager.requirePrivilege()
        }
    }
    ...
    private fun handleSecurityViolation(
        ...
        activity.showSecurityMessage(message)
        eventBridge.emit("security_notice", mapOf("message" to message))
        if (callbackId != null) {
            eventBridge.emit(
                "command_output",
                mapOf(
    ...
    private inline fun guard(
        ...
        try {
            requireCapability(capability, privilegeRequired)
            block()
        } catch (e: SecurityException) {
            handleSecurityViolation(e.message ?: "Action blocked by security policy.", callbackId)
        }
    }

    private inline fun <T> guardValue(
        capability: Capability,
        privilegeRequired: Boolean = false,
        callbackId: String? = null,
        fallback: T,
    ...
    private fun errorJson(message: String): String = gson.toJson(mapOf("error" to message))
    ...
    @JavascriptInterface
    fun showTerminal() {
        guard(Capability.TERMINAL_CONTROL) {
    ...
    fun createSession(): String =
        guardValue(
            capability = Capability.TERMINAL_CONTROL,
            fallback = errorJson("Unable to create terminal session."),
        ) {
            val session = sessionManager.createSession()
            gson.toJson(mapOf("id" to session.mHandle, "name" to (session.title ?: "Terminal")))
        }
    ...
    fun getTerminalSessions(): String =
        guardValue(
            ...
            gson.toJson(sessionManager.getSessionsInfo())
        }
    ...
    fun writeToTerminal(
        ...
        guard(Capability.RUN_ARBITRARY_COMMAND, privilegeRequired = true) {
            activity.confirmPrivilegedCommand(data) {
                val session =
                    if (id.isBlank()) {
    ...
    fun runInNewSession(command: String) {
        guard(Capability.RUN_ARBITRARY_COMMAND, privilegeRequired = true) {
            activity.confirmPrivilegedCommand(command) {
                val session = sessionManager.createSession()
                activity.showTerminal()
    ...
    fun openSetupTerminal() {
        guard(Capability.SETUP) {
            activity.openSetupTerminal()
        }
    }
    ...
    fun getSetupStatus(): String =
        guardValue(
            ...
            fallback = errorJson("Unable to read setup status."),
        ) {
    ...
    fun getBootstrapStatus(): String =
        guardValue(
            capability = Capability.READ_STATUS,
            fallback = errorJson("Unable to read bootstrap status."),
        ) {
            gson.toJson(
                mapOf(
    ...
    fun getActivePlatform(): String =
        guardValue(
            capability = Capability.READ_STATUS,
            fallback = errorJson("Unable to read platform state."),
        ) {
            val markerFile = java.io.File(bootstrapManager.homeDir, ".openclaw-android/.platform")
            val id = if (markerFile.exists()) markerFile.readText().trim() else "openclaw"
            gson.toJson(mapOf("id" to id, "name" to id.replaceFirstChar { it.uppercase() }))
        }
    ...
    fun isToolInstalled(id: String): String {
        return guardValue(
            capability = Capability.READ_STATUS,
            fallback = errorJson("Unable to inspect tool state."),
        ) {
            val prefix = bootstrapManager.prefixDir.absolutePath
            val env = EnvironmentBuilder.build(activity)
            val exists =
            ...
            gson.toJson(mapOf("installed" to exists))
        }
    }
    ...
    fun runCommand(cmd: String): String {
        return guardValue(
            capability = Capability.RUN_SAFE_COMMAND,
            fallback = errorJson("Command blocked."),
        ) {
            if (!CommandRunner.isSafeInspectionCommand(cmd)) {
                throw SecurityException("Only allowlisted inspection commands are available without Advanced Mode.")
            }
            val env = EnvironmentBuilder.build(activity)
    ...
    fun runCommandAsync(
        ...
        guard(
            capability = Capability.RUN_ARBITRARY_COMMAND,
            privilegeRequired = true,
            callbackId = callbackId,
        ) {
            activity.confirmPrivilegedCommand(cmd) {
                launchWithErrorHandling(
    ...
    fun getApkUpdateInfo(): String {
        return guardValue(
            capability = Capability.READ_STATUS,
            fallback = errorJson("Unable to check APK updates."),
        ) {
            try {
                ...
                val tagName =
                    release?.get("tagName") as? String
                        ...
                        ?: return@guardValue gson.toJson(mapOf("error" to "no tag"))
    ...
    private suspend fun updateWww() {
        try {
            ...
            val component =
                resolver.getWwwComponent()
                    ?: throw SecurityException("No signed www update metadata is available.")
            val expectedSha256 =
                component.sha256?.takeIf { it.isNotBlank() }
                    ?: throw SecurityException("Missing SHA-256 for www update.")
            ...
            java.net.URL(component.url).openStream().use { input ->
                zipFile.outputStream().use { output -> input.copyTo(output) }
            }
            BootstrapManager.verifySha256(zipFile, expectedSha256)
    ...
    private fun extractZipEntry(
        ...
        val destFile = secureZipDestination(targetDir, entry.name)
    ...
    fun getAppInfo(): String {
        return guardValue(
            capability = Capability.READ_STATUS,
            fallback = errorJson("Unable to read app info."),
        ) {
    ...
    fun getBatteryOptimizationStatus(): String {
        return guardValue(
            capability = Capability.READ_STATUS,
            fallback = errorJson("Unable to read battery optimization status."),
        ) {
    ...
    fun requestBatteryOptimizationExclusion() {
        guard(Capability.OPEN_SETTINGS) {
            activity.runOnUiThread {
    ...
    fun getStorageInfo(): String {
        return guardValue(
            capability = Capability.READ_STATUS,
            fallback = errorJson("Unable to read storage info."),
        ) {
    ...
    @JavascriptInterface
    fun getPrivilegeStatus(): String =
        guardValue(
            capability = Capability.READ_STATUS,
            fallback = errorJson("Unable to read privilege status."),
        ) {
            gson.toJson(activity.getPrivilegeStatus())
        }

    @JavascriptInterface
    fun requestAdvancedMode() {
        guard(Capability.READ_STATUS) {
            activity.requestAdvancedMode()
        }
    }
    ...
    private fun secureZipDestination(
        ...
        val destination = java.io.File(targetDir, entryName)
        val canonicalTargetDir = targetDir.canonicalFile
        val canonicalDestination = destination.canonicalFile
        val allowedPrefix = canonicalTargetDir.path + java.io.File.separator
        if (
